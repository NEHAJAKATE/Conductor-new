...
exports.identityResolutionService = exports.IdentityResolutionService = void 0;
...
class IdentityResolutionService {
    ...
    resolveIdentity(record, existingLinks // maps lookupKeys (email, phone, govId, customerId) to goldenUuid
    ) {
        ...
        if (!resolvedUuid) {
            ...
            resolvedUuid = uuid_service_1.UuidService.generateUnifiedUuid(anchor);
        ...
        if (email)
            existingLinks.set(`email:${email}`, resolvedUuid);
        if (phone)
            existingLinks.set(`phone:${phone}`, resolvedUuid);
        if (pan)
            existingLinks.set(`pan:${pan}`, resolvedUuid);
        if (aadhaar)
            existingLinks.set(`aadhaar:${aadhaar}`, resolvedUuid);
        if (passport)
            existingLinks.set(`passport:${passport}`, resolvedUuid);
        if (customerId)
            existingLinks.set(`cust:${customerId}`, resolvedUuid);
