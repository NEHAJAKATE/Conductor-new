...
const inventory_ledger_service_1 = require("@/core/inventory/inventory-ledger.service");
class ReportService {
    /**
    ...
    static async generateSalesReport(params) {
        const allTx = await canonical_repositories_1.transactionRepository.getAll();
        ...
        if (params.search) {
            ...
            sales = sales.filter(s => s.partyName.toLowerCase().includes(q) ||
     ...
     */
    static async generatePurchasesReport(params) {
        const allTx = await canonical_repositories_1.transactionRepository.getAll();
        let purchases = allTx.filter(t => t.type === 'purchase' || t.type === 'purchase_return');
        if (params.startDate)
            purchases = purchases.filter(p => p.date >= params.startDate);
        if (params.endDate)
            purchases = purchases.filter(p => p.date <= params.endDate);
        if (params.search) {
            const q = params.search.toLowerCase().trim();
            purchases = purchases.filter(p => p.partyName.toLowerCase().includes(q) ||
     ...
     */
    static async generateOutstandingReport(params) {
        const rawList = await canonical_repositories_1.outstandingRepository.list();
        const isNotConnected = rawList.length === 0;
        let list = await canonical_repositories_1.outstandingRepository.list({
    ...
    static async generateInventoryReport(params) {
        const rawOpeningList = await canonical_repositories_1.inventoryRepository.list();
     ...
     */
    static async generateBusinessActivityReport(params) {
        const businesses = await business_repository_1.businessRepository.list();
        const isNotConnected = businesses.length === 0;
        const stats = await business_repository_1.businessRepository.getStats();
