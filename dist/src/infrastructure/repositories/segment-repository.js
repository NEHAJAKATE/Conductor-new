...
class SegmentRepository {
     ...
     */
    async matchProfiles(rules) {
        const allProfiles = await customer_repository_1.customerRepository.list();
        return allProfiles.filter(p => this.evaluateRules(p, rules));
    }
    evaluateRules(profile, rules) {
        if (rules.length === 0)
            return false;
    ...
    evaluateSingleRule(profile, rule) {
        ...
        switch (field) {
            ...
            case 'campaign': {
                const campaign = profile.behavioralEvents.find(e => e.campaign)?.campaign || '';
                return this.compareValues(campaign, operator, value);
            }
            case 'device': {
                const device = profile.behavioralEvents.find(e => e.device)?.device || '';
