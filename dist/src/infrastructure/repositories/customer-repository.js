...
const cdp_repositories_1 = require("./cdp-repositories");
...
class CustomerRepository {
    ...
    async list() {
        ...
        const behaviors = await cdp_repositories_1.behaviorRepository.list();
        const financials = await cdp_repositories_1.financialRepository.list();
        ...
        for (const record of identities) {
            ...
            for (const p of Array.from(profilesMap.values())) {
                ...
                if (exactIdMatch || exactEmailMatch || exactPhoneMatch || exactPanMatch || exactAadhaarMatch || exactPassportMatch || compositeMatch) {
                    ...
                    const reasons = [];
                    if (exactIdMatch)
                        reasons.push('Customer ID');
                    if (exactEmailMatch)
                        reasons.push('Email Address');
                    if (exactPhoneMatch)
                        reasons.push('Phone Number');
                    if (exactPanMatch)
                        reasons.push('PAN Card');
                    if (exactAadhaarMatch)
                        reasons.push('Aadhaar Card');
                    if (exactPassportMatch)
            ...
            const uuid = matchedUuid || (0, uuid_1.v5)(canonicalSeed, config_1.config.identity.namespace);
            ...
            if (existingProfile) {
                ...
                const newProfile = {
                    ...
                    identity: {
                        ...
                        segment: (record.segment === 'VIP' ? 'VIP' : 'Regular'),
                        riskScore: 10,
                        completionRate: 75,
                        status: 'Active',
        ...
        for (const profile of Array.from(profilesMap.values())) {
            ...
            const matchedEvents = behaviors.filter(b => (email && b.email?.toLowerCase() === email) ||
                (customerId && b.customerId === customerId));
            profile.behavioralEvents = matchedEvents.map(b => {
                let mappedType = 'Visited Website';
                const typeLower = b.type.toLowerCase();
                if (typeLower.includes('visit') || typeLower.includes('website'))
                    mappedType = 'Visited Website';
                else if (typeLower.includes('cart'))
                    mappedType = 'Added Cart';
                else if (typeLower.includes('purchase') || typeLower.includes('buy') || typeLower.includes('paid'))
                    mappedType = 'Purchased';
                else if (typeLower.includes('refund'))
                    mappedType = 'Refund';
                else if (typeLower.includes('ticket') || typeLower.includes('support'))
                    mappedType = 'Support Ticket';
                else if (typeLower.includes('email') || typeLower.includes('open'))
                    mappedType = 'Email Open';
                else if (typeLower.includes('bank') || typeLower.includes('transaction'))
                    mappedType = 'Bank Transaction';
                return {
                    eventId: b.eventId,
                    type: mappedType,
        ...
        for (const profile of Array.from(profilesMap.values())) {
            const email = profile.identity.email?.toLowerCase();
            const customerId = profile.identity.customerId;
            const matchedFin = financials.filter(f => (email && f.email?.toLowerCase() === email) ||
            ...
            if (matchedFin.length > 0) {
                profile.financial.invoices = matchedFin.map(f => ({
                    ...
                    dueDate: f.date,
                    amount: f.amount,
                    status: (f.status === 'Paid' ? 'Paid' : f.status === 'Unpaid' ? 'Unpaid' : 'Overdue')
                }));
        ...
        for (const profile of Array.from(profilesMap.values())) {
            ...
            if (profile.identity.email)
                filled++;
            if (profile.identity.phone && profile.identity.phone !== 'Not Linked')
                filled++;
            if (profile.identity.dob)
                filled++;
            if (profile.identity.address)
                filled++;
            if (profile.identity.pan && profile.identity.pan !== 'Not Linked')
                filled++;
            if (profile.identity.aadhaar && profile.identity.aadhaar !== 'Not Linked')
                filled++;
            if (profile.identity.passport && profile.identity.passport !== 'Not Linked')
                filled++;
    ...
    async search(query) {
        ...
        return list.filter(p => {
            return (p.uuid.toLowerCase().includes(q) ||
    ...
    async getStats() {
        ...
        list.forEach(p => {
            ...
            if (type === 'Known Customer')
                knownCustomers++;
            else if (type === 'Anonymous Visitor')
                anonymousVisitors++;
            else
                resolvedProfiles++; // Unified Profile count
            if (p.behavioralEvents.length > 2)
                activeVisitors++;
            if (p.behavioralEvents.length > 0)
                returningVisitors++;
        ...
        const idList = await cdp_repositories_1.identityRepository.list();
    ...
    async mergeAnonymousVisitor(anonymousCustomerId, email, name) {
        const list = await this.list();
        ...
        if (anonymousProfile) {
            if (knownProfile) {
                ...
                const records = await cdp_repositories_1.identityRepository.list();
                ...
                await cdp_repositories_1.identityRepository.delete(anonymousProfile.identity.customerId || '');
                return knownProfile;
            }
            else {
