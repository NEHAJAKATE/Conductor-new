...
export interface MockIdentity {
  ...
  country: string;
  uuid: string;
  segment: 'Regular' | 'VIP' | 'Churn-Risk' | 'Inactive';
}
...
export interface MockBehavior {
  ...
  geo: string;
  referral: 'Google' | 'Direct' | 'Facebook' | 'LinkedIn' | 'Newsletter';
}
...
const REFERRALS = ['Google', 'Direct', 'Facebook', 'LinkedIn', 'Newsletter'] as const;
...
export class MockarooGenerator {
  generateIdentities(count: number = 20): MockIdentity[] {
    const list: MockIdentity[] = [];
    for (let i = 0; i < count; i++) {
      ...
      let pan = '';
      for (let j = 0; j < 5; j++) pan += panChars[Math.floor(Math.random() * panChars.length)];
      ...
      const uuid = uuidv5(`email:${email.toLowerCase().trim()}`, config.identity.namespace);
  ...
  generateBehavioral(identities: any[], count: number = 50): MockBehavior[] {
    const list: MockBehavior[] = [];
    const eventTypes = [
    ...
    for (let i = 0; i < count; i++) {
      ...
      if (type === 'Visited Website') details = `Visited page path: /products/${campaign.toLowerCase().replace(/ /g, '-')}`;
      else if (type === 'Purchased') details = `Checked out order. Campaign referral: ${campaign}`;
      else if (type === 'Support Ticket') details = `Created support request regarding invoice validation.`;
  ...
  generateFinancial(identities: any[], count: number = 30): MockFinancial[] {
    const list: MockFinancial[] = [];
...
export const mockarooGenerator = new MockarooGenerator();
