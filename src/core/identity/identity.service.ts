...
export class IdentityService {
  async resolve(records: Array<Record<string, any>>): Promise<{
    ...
    const emailToProfileMap = new Map<string, MasterProfile>();
    const phoneToProfileMap = new Map<string, MasterProfile>();
    const idToProfileMap = new Map<string, MasterProfile>();
    const crmIdToProfileMap = new Map<string, MasterProfile>();
    const customerIdToProfileMap = new Map<string, MasterProfile>();
    const compositeToProfileMap = new Map<string, MasterProfile>();
    ...
    for (const record of records) {
      ...
      let matchedProfile: MasterProfile | undefined;
      ...
      if (matchedProfile) {
        ...
        if (crmId && !matchedProfile.crmId) matchedProfile.crmId = crmId;
        if (customerId && !matchedProfile.customerId) matchedProfile.customerId = customerId;

        // Recalculate confidence score based on intersection vectors (0 - 100)
        let reasons: string[] = matchedProfile.matchingReason.split(', ');
        ...
        if (email) emailToProfileMap.set(email, newProfile);
        if (phone) phoneToProfileMap.set(phone, newProfile);
        if (id) idToProfileMap.set(id, newProfile);
        if (crmId) crmIdToProfileMap.set(crmId, newProfile);
        if (customerId) customerIdToProfileMap.set(customerId, newProfile);
        if (compositeKey) compositeToProfileMap.set(compositeKey, newProfile);
      }
    }
